...
def Conv2d(in_planes, out_planes, kernel_size, stride=1, padding=1, args=None, logger=None):
    """convolution"""
    global name
    if args.mode == "WAGE":
...
class BasicBlock_customK(nn.Module):
    ...
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = Conv2d(inplanes, planes, kernel_size, stride, kernel_size//2, args=args, logger=logger)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = Conv2d(planes, planes, kernel_size, padding=kernel_size//2, args=args, logger=logger)
        self.bn2 = norm_layer(planes)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, args, logger, num_classes=10, zero_init_residual=False,
        ...
        self.conv1 = Conv2d(3, self.inplanes, kernel_size=3, stride=1, padding=1, args=args, logger=logger)
...
class ResNet_custom(nn.Module):
    ...
    def __init__(self, block, layers, args, logger, num_classes=10, zero_init_residual=False,
        ...
        self.relu = nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 16, 5, layers[0], args=args)
        self.layer2 = self._make_layer(block, 32, 5, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], args=args, logger=logger)
        self.layer3 = self._make_layer(block, 16, 5, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], args=args, logger=logger)
        # self.layer4 = self._make_layer(block, 16, 3, layers[3], stride=2,
        #                                dilate=replace_stride_with_dilation[2], args=args, logger=logger)
    ...
    def _make_layer(self, block, planes, kernel_size, blocks, stride=1, dilate=False, args=None, logger=None):
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:

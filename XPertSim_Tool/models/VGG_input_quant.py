...
from modules.quantization_cpu_np_infer_inp_quant import QConv2d,  QLinear
...
class VGG(nn.Module):
    ...
    def forward(self, x, dac_prec, adc_prec, batch, model):
        print("ye hua")
        count = 0
        ...
        for i in self.features:
            if isinstance(i, FConv2d):
                ...
                # if batch == 0:
                #     torch.save(intermed_out, './'+model+'_outs/output' + str(count))
                count += 1
                print(count)
                # print('this')
            else:
...
def make_layers(cfg, args, logger, mode):
    layers = []
    in_channels = 3
...
cfg_list = {
    ...
    '14.9mm2_model': [('C', 16, 3, 'same', 8), ('M', 2, 2), ('C', 16, 3, 'same', 8), ('C', 32, 3, 'same', 8), ('C', 128, 3, 'same', 8), ('M', 2, 2), ('C', 256, 3, 'same', 8), ('C', 128, 3, 'same', 8), ('C', 128, 3, 'same', 8), ('M', 2, 2), ('C', 256, 3, 'same', 8), ('C', 16, 3, 'same', 8), ('C', 16, 3, 'same', 8), ('C', 256, 3, 'same', 8), ('C', 32, 3, 'same', 8)],

...
class FConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, logger=None,
    ...
    def forward(self, input, dac, adc):
        if self.inference == 1:
            ...
            input = float_quantizer.float_range_quantize(input,dac) #self.wl_input)
            output= F.conv2d(input, weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
            output = wage_quantizer.LinearQuantizeOut(output, adc) #self.ADCprecision)
        else:
            # print('inference 0')
...
class FLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True,logger = None,
	             wl_input =8,wl_weight= 8,inference=0,onoffratio=10,cellBit=1,subArray=128,ADCprecision=5,

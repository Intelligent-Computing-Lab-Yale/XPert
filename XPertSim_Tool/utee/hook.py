...
def Neural_Sim(self, input, output): 
    ...
    if len(self.weight.shape) > 2:
        ...
        write_matrix_activation_fc(torch.ones_like(input[0]).cpu().data.numpy(),None ,self.wl_input, input_file_name)
...
def stretch_input(input_matrix,window_size = 5,padding=(0,0),stride=(1,1)):
    ...
    print('hook running')
    print(f' input_shape {input_shape}, window_size {window_size}, padding {padding}, stride {stride}')
    # print(window_size, padding, stride)
    ...
    print(int((input_shape[2] + 2 * padding[0] - window_size) / stride[0]+1)-window_size+1)
    for i in range( int((input_shape[2] + 2*padding[0] - window_size) / stride[0] + 1)-window_size+1):
        for j in range( int((input_shape[3] + 2*padding[1] - window_size) / stride[1] + 1) -window_size+1):
            for b in range(input_shape[0]):
                # print(input_matrix[b, :, i:i+window_size,j: j+window_size].shape)
